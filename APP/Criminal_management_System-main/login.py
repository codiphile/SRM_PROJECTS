...
class Login_Window:
    def __init__(self, root):
        ...
        # self.bg = ImageTk.PhotoImage(file=r"images\2.jpeg")
        # lbl_bg = Label(self.root, image=self.bg)
        # lbl_bg.place(x=0, y=0, relwidth=1, relheight=1)

        # self.bg_image = Image.open("images/2.jpeg")
        ...
        if self.txtuser.get() == "" or self.txtpass.get() == "":
            ...
            try:
                con = mysql.connector.connect(
                    host="localhost",
                    username="root",
                    password="Unstoppable@1",
                    database="management",
                )
                cur = con.cursor()
                cur.execute(
                    "select * from login where userid=%s and password=%s",
